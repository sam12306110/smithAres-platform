...
(function () {
    ...
    angular.module('BlurAdmin.pages').controller('memberOutCheck', memberOutCheck);

    /** @ngInject */
    function memberOutCheck($scope, httpFactory, lotteryConst, custNotify, $uibModal, todoService, $interval, common, $sce) {
        ...
        $scope.getAudio = function () {
            httpFactory.getList(lotteryConst.aresPathPlat + '/soundConfig' + '/view', 'GET', null, {itemType: 3}).then(function (result) {
                if (result.data) {
        ...
        $scope.onSearch = function (e) {
            ...
            httpFactory.getList(url + '/applyList', 'GET', null, $scope.queryParam).then(function (result) {
                $scope.entity = result.data.rows;
                for (var i = 0; i < $scope.entity.length; i++) {
                    //$scope.entity[i].push({flag:false})
        ...
        $scope.copytext = function (e) {
            ...
            //提示
            if (successful) {
                custNotify.success(e.target.innerText, '已成功复制:');
            } else {
                custNotify.error('浏览器不支持复制', '提示:');
            }
        };
        ...
        $scope.openEdit = function (item) {
            ...
            open('app/pages/funds/memberOutCheck/memberOutCheckCancel.html', 'md');
        };
        $scope.openAudit = function (item) {
            todoService.set({
                item: item
            });
            open('app/pages/funds/memberOutCheck/audit.html', 'md');
        }
