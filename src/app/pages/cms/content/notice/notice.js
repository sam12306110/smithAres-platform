...
(function () {
    ...
    angular.module('BlurAdmin.pages').controller('notice', notice);

    /** @ngInject */
    function notice($scope, httpFactory, lotteryConst, custNotify, $uibModal, todoService, common) {
        ...
        $scope.onStatus = function (item) {
            ...
            httpFactory.getList(url + '/save', 'POST', null, item).then(function (result) {
                if (result.data == null) {
                    custNotify.warning('启用数量已达到10条', '提示：');
                } else {
        ...
        $scope.openEdit = function (edit, item, num) {
            var num = num || 0;
            todoService.set({
                edit: edit,
                item: item,
            ...
            open('app/pages/cms/content/notice/noticeEdit.html', 'md');
        };
    }
})();
