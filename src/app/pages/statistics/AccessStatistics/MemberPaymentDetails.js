...
(function () {
    ...
    angular.module('BlurAdmin.pages').controller('MemberPaymentDetails',MemberPaymentDetails);

    /** @ngInject */
    function MemberPaymentDetails($scope, httpFactory, lotteryConst, custNotify, $interval, todoService, common, $sce) {
        ...
        function initPage() {
            $scope.selectParam = {
                ...
                dispensingType:[{id: null, value: '全部'}].concat(lotteryConst.dispensing),
        ...
        $scope.onSearch = function (e) {
            var mbName=window.localStorage.getItem('memberName')
            if (!e) {
            ...
            if($scope.selectParam.statusSelected){
                $scope.queryParam.operateType=$scope.selectParam.statusSelected.id
            }
            if(mbName){
                $scope.queryParam.memberName=mbName
            }
            httpFactory.getList(url+ '/momey/statistics/outList', 'GET', null, $scope.queryParam).then(function (result) {
                if(result.data){
