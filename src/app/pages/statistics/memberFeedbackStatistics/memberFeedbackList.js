...
(function () {
    ...
    angular.module('BlurAdmin.pages').controller('memberFeedbackList', memberFeedbackList);

    /** @ngInject */
    function memberFeedbackList($scope, httpFactory, lotteryConst, custNotify, todoService, $uibModal, common, $sce) {
        $scope.pageSize = lotteryConst.pageSize; //页面显示行
        var url = lotteryConst.paymentPlatPath;

        function initPage() {
            $scope.selectParam = {
                startTime: todoService.get().startTime || localStorage.cashStartTime,
                endTime: todoService.get().endTime || localStorage.cashEndTime,
            ...
            $scope.queryParam = {
                ...
                agentId: todoService.get().agentId || localStorage.cashAgentId
        ...
        $scope.onSearch = function (e) {
            ...
            httpFactory.getList(url + '/member/backwater/stats', 'GET', null, $scope.queryParam).then(function (result) {
        ...
        $scope.gogogo = function (e) {
            ...
            window.localStorage.setItem('cashAgentId2', e.agentId);
            window.localStorage.setItem('cashMemberId2', e.memberId);
            window.localStorage.setItem('cashStartTime2', $scope.selectParam.startTime);
            window.localStorage.setItem('cashEndTime2', $scope.selectParam.endTime);
            window.location.href = '#/statistics/memberFeedbackDetails'
        };
