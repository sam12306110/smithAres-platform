...
(function () {
    ...
    angular.module('BlurAdmin.pages').controller('platOfficialEditCtrl', platOfficialEditCtrl);

    /** @ngInject */
    function platOfficialEditCtrl($scope, $rootScope, httpFactory, lotteryConst, custNotify, $filter) {
        var banks = [{
            id: "icbc", value: "中国工商银行"
        }, {
            id: "abc", value: "中国农业银行"
        }];
        $scope.selectParam = {
        ...
        var lotteries = $filter('filter')(lotteryConst.lottery, {sideType: 1});
        ...
        $.when(
            //加载赔率
            $.ajax({
                url: lotteryConst.aresPath + '/odds/getOddsMap?lotteryIds=' + lotteryIds,
                type: 'GET'
            }),
            //现金方案列表
            $.ajax({
                url: lotteryConst.aresPath + '/settlePlan/getListMap?settleType=1&sideType=1',
                type: 'GET'
            }),
            //信用结算方案列表
            $.ajax({
                url: lotteryConst.aresPath + '/settlePlan/getListMap?settleType=2&sideType=1',
                type: 'GET'
            }))
        ...
        function save() {
            ...
            if ($scope.selectParam.banksSelected != null) {
                $scope.entity.bank = $scope.selectParam.banksSelected.id;
            }
            if (pOdds) {
